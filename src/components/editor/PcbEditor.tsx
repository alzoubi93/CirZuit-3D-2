...
import {
  ...
  Layers as LayersIcon, Settings2, Zap, AlertTriangle, ShieldCheck, Box, RotateCw, Palette,
...
function PcbEditorInner({
  ...
  highlightedNetIds,
  onUndo, onRedo, canUndo, canRedo,
  hasEcoChanges, setEcoOpen, setMode, setUiFootprintGenOpen, setUiLayersOpen, uiFootprintGenOpen, uiLayersOpen,
}: {
  schematic: import("@/lib/schematic").SchematicDoc;
  pcb: PcbDoc;
  ...
  useEffect(() => {
    if (pcb.isImportedGerber) {
      setDrcOpen(false);
    }
  }, [pcb.isImportedGerber]);
  ...
  const ratsnest = useMemo(() => computeRatsnest(deferredSchematic, deferredPcb), [deferredSchematic, deferredPcb]);
  ...
  const processPointerMove = () => {
    ...
    if (activePointers.current.size === 1) {
      ...
      if (dragGroup && dragGroup.start) {
        ...
        if (!dragGroup.moved) {
          if (isTouch && Math.hypot(dx, dy) < 1.5) {
            return; // Wait for clear movement intent to prevent accidental drags during pinch gestures
          }
          commitHistory();
        ...
        const grid = pcb.gridMm || 0.5;
        const rawDx = dx;
        const rawDy = dy - ((isTouch && !pcb.isImportedGerber) ? 15 : 0);
  ...
  const visibleTracks = useMemo(() => {
    const tracks = deferredPcb.tracks || [];
  ...
  }, [deferredPcb.tracks, viewportBounds]);

  const visibleFootprints = useMemo(() => {
    if (!viewportBounds) return deferredPcb.footprints || [];
    const { minX, maxX, minY, maxY } = viewportBounds;
    return (deferredPcb.footprints || []).filter(f => 
      f.x >= minX - 50 && f.x <= maxX + 50 &&
      f.y >= minY - 50 && f.y <= maxY + 50
    );
  }, [deferredPcb.footprints, viewportBounds]);

  const visibleVias = useMemo(() => {
    if (!viewportBounds) return deferredPcb.vias || [];
    const { minX, maxX, minY, maxY } = viewportBounds;
    return (deferredPcb.vias || []).filter(v => 
      v.x >= minX && v.x <= maxX &&
      v.y >= minY && v.y <= maxY
    );
  }, [deferredPcb.vias, viewportBounds]);

  const visibleTexts = useMemo(() => {
    if (!viewportBounds) return deferredPcb.texts || [];
    const { minX, maxX, minY, maxY } = viewportBounds;
    return (deferredPcb.texts || []).filter(t => 
      t.x >= minX - 10 && t.x <= maxX + 10 &&
      t.y >= minY - 10 && t.y <= maxY + 10
    );
  }, [deferredPcb.texts, viewportBounds]);

  const visiblePads = useMemo(() => {
    if (!viewportBounds) return deferredPcb.pads || [];
    const { minX, maxX, minY, maxY } = viewportBounds;
    return (deferredPcb.pads || []).filter(p => 
      p.x >= minX - 10 && p.x <= maxX + 10 &&
      p.y >= minY - 10 && p.y <= maxY + 10
    );
  }, [deferredPcb.pads, viewportBounds]);
  ...
  return (
    <div className="flex flex-col h-full bg-background">
      <div className="flex items-center gap-0 px-0 h-9 border-b bg-card shrink-0 overflow-x-auto">
        ...
        <Button
          size="sm"
          onClick={() => setUiFootprintGenOpen(true)}
          title={lang === "ar" ? "توليد البصمات" : "Footprint Generator"}
          className="h-7 px-1 gap-0.5 text-xs bg-black hover:bg-black/80 text-white border border-slate-700"
        >
          <LayoutGrid className="size-4" />
          <span className="hidden sm:inline text-xs">{lang === "ar" ? "بصمات" : "Footprints"}</span>
        </Button>
        ...
        {!pcb.isImportedGerber && (
          <Button
            ...
            title={lang === "ar" ? "فحص قواعد التصميم (DRC)" : "Design Rule Check (DRC)"}
            className={`h-7 px-1.5 gap-1 text-xs transition-all relative shrink-0 ${
              drcOpen
                ? "bg-amber-600/40 text-white border border-amber-500/50"
                : drcErrors.length > 0
                  ? "bg-red-950/40 text-red-400 border border-red-900/50 hover:bg-red-900/20"
                  : "bg-black text-white hover:bg-black/80 border border-slate-700"
            }`}
          >
            {drcErrors.length > 0 ? (
              <AlertTriangle className={`size-4 ${drcErrors.length > 0 ? "text-amber-400 animate-pulse" : ""}`} />
            ) : (
            ...
            {drcErrors.length > 0 && (
              <span className="flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[9px] font-bold text-white px-1 shrink-0">
                {drcErrors.length}
              </span>
            )}
          </Button>
        ...
        <div className="ml-auto flex items-center gap-0.5 pr-1">
          {setEcoOpen && !pcb.isImportedGerber && (
            <Button
              size="sm"
              variant={hasEcoChanges ? "destructive" : "outline"}
              className={`h-7 px-2 rounded-md text-xs font-bold gap-0.5 transition-all duration-300 relative ${
                hasEcoChanges ? "animate-pulse border-red-500 bg-red-500/15 hover:bg-red-500/25 text-red-500" : ""
              }`}
              onClick={() => setEcoOpen(true)}
              title="Engineering Change Order (ECO) Sync"
            >
              <span>ECO</span>
              {hasEcoChanges && (
      ...
      <div className="flex-1 min-h-0 flex relative">
                                ...
                                if (isSmd) {
                                  return (
                                    <g style={{ pointerEvents: "none" }} transform={`translate(${nonPolarCx}, ${nonPolarCy}) rotate(${angle})`}>
                                       ...
                                       <rect x={-rectW/2} y={-rectH/2} width={rectW * 0.2} height={rectH} fill={silkColor} />
                      ...
                      {(() => {
                        ...
                        return fp.pads.map((pad, idx) => {
                          ...
                          const padColor = isPadSelected 
                            ? "#60a5fa" 
                            : (padLayer?.color || (pad.layer === "bottom_copper" 
                                ? "#3b82f6" 
                                : "#ef4444"));
                          return (
                ...
                <div className="space-y-4 pt-4 border-t border-border/50">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-bold uppercase tracking-widest text-amber-500 flex items-center gap-1.5">
                      ...
                      {lang === "ar" ? "قواعد التصميم" : "DRC Report"}
                    </span>
                  ...
                  <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
                    {drcErrors.length === 0 ? (
                      <div className="p-3 bg-emerald-500/5 border border-emerald-500/20 rounded-lg text-[11px] text-emerald-600 font-medium">
                        ✓ {lang === "ar" ? "لا توجد أخطاء في التصميم" : "Design is clear!"}
                      </div>
                    ) : (
                      drcErrors.map((err) => (
                        <div
                          key={err.id}
                          onClick={() => {
                            setActiveDrcError(err);
                            if (svgRef.current) {
                              const r = svgRef.current.getBoundingClientRect();
                              setPan({ x: r.width / 2 - err.x * zoom, y: r.height / 2 - err.y * zoom });
                            }
                          }}
                          className={`p-2.5 rounded-lg border text-[11px] cursor-pointer transition-all ${
                            activeDrcError?.id === err.id
                              ? "bg-red-500/10 border-red-500/50 ring-1 ring-red-500/20 shadow-sm"
                        ...
                        >
                          <div className="font-bold flex items-center gap-1.5 mb-1 text-foreground">
                            ...
                            <span className="capitalize">{err.type}</span>
                          </div>
                          <p className="text-muted-foreground leading-tight text-[10px]">{err.msg}</p>
                        </div>
                      ))
                    )}
                  </div>
        ...
        {uiFootprintGenOpen && (
          <aside ref={footprintGenRef} className="w-80 md:w-96 border-s bg-slate-900/95 backdrop-blur-md flex flex-col overflow-hidden absolute md:relative inset-y-0 end-0 z-40 md:z-auto shadow-lg md:shadow-none transition-all border-l border-slate-800">
            <div className="px-4 py-3 border-b flex items-center justify-between bg-slate-900/50 shrink-0 border-b border-slate-800">
              <div className="font-semibold text-sm flex items-center gap-2">
                <Cpu className="size-4 text-primary animate-pulse" />
                {lang === "ar" ? "توليد البصمات" : "Footprint Generator"}
              </div>
              <Button variant="ghost" size="icon" onClick={() => setUiFootprintGenOpen(false)} className="h-8 w-8 text-blue-400 hover:text-white hover:bg-blue-600/20 border-2 border-blue-500/80 hover:border-blue-400 rounded-lg transition-all flex items-center justify-center shadow-sm shadow-blue-500/20">
                <X className="size-4 stroke-[2.5]" />
              </Button>
            </div>

            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground border-b pb-1.5">{lang === "ar" ? "إنشاء بصمة عنصر جديدة" : "Create New Component Footprint"}</div>
              
              <div className="space-y-3">
                <div className="space-y-1.5">
                  <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">{lang === "ar" ? "نوع الحزمة" : "Package Type"}</Label>
                  <select
                    className="w-full h-9 rounded-md border border-input bg-background/50 px-2 py-1 text-xs shadow-sm transition-colors focus:outline-none focus:ring-1 focus:ring-ring"
                    value={genType}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">{lang === "ar" ? "البادئة" : "Prefix"}</Label>
                    <Input
                  ...
                  <div className="space-y-1.5">
                    <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">{lang === "ar" ? "القيمة" : "Value"}</Label>
                    <Input
                      className="h-9 text-xs bg-background/50"
                      value={genValue}
                ...
                {genType !== "Passive" && (
                  <div className="space-y-1.5">
                    <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">{lang === "ar" ? "عدد الأرجل" : "Pin Count"}</Label>
                    <Input
                      type="number"
                      className="h-9 text-xs bg-background/50"
                      value={genPinCount}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <BoardNumberInput
                    label={lang === "ar" ? "الخطوة (Pitch)" : "Pitch"}
                    value={genPitch}
                    unit={pcb.unit}
                    onChange={(v) => setGenPitch(v)}
                  />

                  {genType !== "Passive" && (
                    <BoardNumberInput
                      label={lang === "ar" ? "تباعد الصفوف" : "Row Spacing"}
                      value={genRowSpacing}
                      unit={pcb.unit}
                      onChange={(v) => setGenRowSpacing(v)}
                    />
                  )}

                  <BoardNumberInput
                    label={lang === "ar" ? "عرض الوسادة" : "Pad Width"}
                    value={genPadWidth}
                    unit={pcb.unit}
                    onChange={(v) => setGenPadWidth(v)}
                  />

                  <BoardNumberInput
                    label={lang === "ar" ? "ارتفاع الوسادة" : "Pad Height"}
                    value={genPadHeight}
                    unit={pcb.unit}
                    onChange={(v) => setGenPadHeight(v)}
                  />

                  {genType === "DIP" && (
                    <BoardNumberInput
                      label={lang === "ar" ? "قطر الثقب" : "Drill"}
                      value={genDrill}
                      unit={pcb.unit}
                      onChange={(v) => setGenDrill(v)}
                    />
                  )}
                </div>
                ...
                >
                  {lang === "ar" ? "توليد وإضافة للوحة" : "Generate Footprint"}
                </Button>
              </div>
            </div>
          </aside>
