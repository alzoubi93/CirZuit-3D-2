...
export function Canvas({
  ...
  const autoConnect = useCallback(
    (d: SchematicDoc, movedNodeId: string): SchematicDoc => {
      ...
      const sym = SYMBOLS[n.symbol];
      if (!sym) return d;
      const pins = transformedPins(sym, n.rotation, n.size).map((p) => ({
        x: n.x + p.x,
        y: n.y + p.y,
      }));
      let wires = d.wires;
  ...
  const onPointerDown = (e: React.PointerEvent) => {
          ...
          if (idx === 0 || idx === w.points.length - 1) {
            for (const n of doc.nodes) {
              ...
              const pins = transformedPins(sym, n.rotation, n.size).map(
                (p) => ({ x: n.x + p.x, y: n.y + p.y }),
              );
              for (const pin of pins) {
                if (Math.hypot(pt.x - pin.x, pt.y - pin.y) < 0.25) {
                  attachedNodeId = n.id;
    ...
    if (placement) {
      ...
      if (placement.multi) {
        ...
        onPlace?.(placement.symbol, snapped.x, snapped.y, placement.rotation ?? 0);
      }
      return;
    }
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {sym.pins.map((_, i) => renderPinLabel(n, i, nodeColor))}
                  {n.reference && (() => {
