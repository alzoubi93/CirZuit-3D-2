...
function migrateLegacyProject(p: any): ProjectRecord {
  if (!p.doc) {
    let migratedPcb = p.pcb;
    if (migratedPcb && !migratedPcb.footprints) {
      migratedPcb = {
        footprints: migratedPcb.components || [],
        traces: migratedPcb.wires || migratedPcb.traces || [],
        boardOutline: migratedPcb.boardOutline || null,
      };
    }
